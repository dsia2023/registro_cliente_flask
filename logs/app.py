...
def get_cursor(dictionary=False):
    try:
        ...
        # Opcional: puedes intentar reabrir la conexión si fue cerrada completamente
            db = connect(
            host="localhost",
            user="root",
            password="Fgn2024$",
    ...
    cursor =  get_cursor(dictionary=True) if dictionary else get_cursor()
    return cursor
...
def contar_mensajes():
    if session.get("admin"):
        db.ping(reconnect=True)  # 👈 Asegura que la conexión esté viva
        cursor = get_cursor()
        cursor.execute("SELECT COUNT(*) FROM mensajes_contacto WHERE leido = 0")
        cantidad = cursor.fetchone()[0]
        session["nuevos_mensajes"] = cantidad > 0
...
def cotizacion_pdf():
    ...
    cursor = cursor = get_cursor(dictionary=True)
    cursor.execute("SELECT direccion FROM clientes WHERE rut = %s", (rut,))
    usuario = cursor.fetchone()
    correo_destino = usuario["direccion"] if usuario else None

    # Renderizar HTML
    html = render_template("cotizacion_pdf.html", carrito=carrito, total=total, cliente=cliente, fecha=fecha)
    ...
    if correo_destino:
        try:
            msg = Message(
                subject="📄 Cotización desde ALLTAK CHILE",
                sender=app.config['MAIL_USERNAME'],
                recipients=[correo_destino]
            )
            msg.body = f"Hola {cliente}, adjuntamos tu cotización generada desde nuestro sitio web."
            msg.attach("cotizacion.pdf", "application/pdf", pdf)
            mail.send(msg)
            flash("✅ Cotización enviada por correo exitosamente.")
        except Exception as e:
            print("❌ Error al enviar correo:", e)
            flash("⚠️ Cotización generada, pero no se pudo enviar por correo.")



    # Retornar el PDF directamente también
